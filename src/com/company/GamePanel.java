...
public class GamePanel extends JPanel implements Runnable, KeyListener {
    ...
    private CollisionLogic head, apple;
    private ArrayList<CollisionLogic> snake;
    private int score;
    private int level;
    ...
    private void setUplevel() {
        snake = new ArrayList<CollisionLogic>();
        head = new CollisionLogic(SIZE);
        head.setPosition(WIDTH / 2, HEIGHT / 2);
        snake.add(head);

        for (int i = 1; i < 3; i++) {
            CollisionLogic e = new CollisionLogic(SIZE);
            e.setPosition(head.getX() + (i * SIZE), head.getY());
            snake.add(e);

        }
        apple = new CollisionLogic(SIZE);
        setApple();
        score = 0;
        gameover = false;
        level = 1;
    ...
    public void render(Graphics2D g2d) {
        ...
        apple.render(g2d);
        if (gameover) {
            g2d.drawString("GameOver!" + level, 150, 200);
        }

        g2d.setColor(Color.WHITE);
        g2d.drawString("Score : " + score + "Level : " + level, 10, 10);
        if (dx == 0 && dy == 0) {
            g2d.drawString("Ready!" + level, 150, 200);
